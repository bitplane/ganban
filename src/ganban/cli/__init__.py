...
def build_parser() -> argparse.ArgumentParser:
    ...
    # --- init ---
    init_p = nouns.add_parser("init", help="Initialize a ganban board")
    init_p.set_defaults(func=init_board)

    # --- board ---
    board_p = nouns.add_parser("board", help="Board operations")
    board_verbs = board_p.add_subparsers(dest="verb")

    board_summary_p = board_verbs.add_parser("summary", help="Show board summary")
    board_summary_p.set_defaults(func=board_summary)

    board_get_p = board_verbs.add_parser("get", help="Dump board index.md")
    board_get_p.set_defaults(func=board_get)

    board_set_p = board_verbs.add_parser("set", help="Write board index.md from stdin")
    ...
    card_move_p = card_verbs.add_parser("move", help="Move a card")
    ...
    card_archive_p = card_verbs.add_parser("archive", help="Archive a card")
    card_archive_p.add_argument("id", help="Card ID")
    card_archive_p.set_defaults(func=card_archive)

    # --- column ---
    col_p = nouns.add_parser("column", help="Column operations")
    col_verbs = col_p.add_subparsers(dest="verb")

    col_list_p = col_verbs.add_parser("list", help="List columns")
    col_list_p.set_defaults(func=column_list)

    col_get_p = col_verbs.add_parser("get", help="Dump column index.md")
    col_get_p.add_argument("id", help="Column ID")
    col_get_p.set_defaults(func=column_get)

    col_set_p = col_verbs.add_parser("set", help="Write column index.md from stdin")
    col_set_p.add_argument("id", help="Column ID")
    col_set_p.set_defaults(func=column_set)

    col_add_p = col_verbs.add_parser("add", help="Create a column")
    ...
    col_move_p = col_verbs.add_parser("move", help="Move a column")
    ...
    col_rename_p = col_verbs.add_parser("rename", help="Rename a column")
    ...
    col_archive_p = col_verbs.add_parser("archive", help="Archive a column")
    col_archive_p.add_argument("id", help="Column ID")
    col_archive_p.set_defaults(func=column_archive)

    # --- reserved ---
    for name in ("sync", "web", "serve"):
        p = nouns.add_parser(name, help=f"{name} (not yet implemented)")
